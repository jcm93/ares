...
struct mToolbar : mWidget {
  ...
  auto onClose(const std::function<void (ToolbarItem)>& callback = {}) -> type&;
  auto onMove(const std::function<void (ToolbarItem, ToolbarItem)>& callback = {}) -> type&;
...
//private:
  struct State {
    ...
    std::function<void (ToolbarItem)> onClose;
    std::function<void (ToolbarItem, ToolbarItem)> onMove;
  } state;
