...
function(_bundle_dependencies target)
  ...
  foreach(library IN LISTS found_dependencies)
    ...
    if(is_imported)
      ...
      _check_library_location(${library} ${imported_location})
    endif()
  endforeach()
...
macro(_check_library_location library location)
  if(library_type STREQUAL "SHARED_LIBRARY")
    ...
    if(location MATCHES ".+lib$")
      ...
      unset(_dll_name)
      unset(_pdb_name)
    elseif(location MATCHES ".+dll\.a$")
      cmake_path(GET location FILENAME _dll_name)
      cmake_path(GET location PARENT_PATH _implib_path)
      cmake_path(SET _bin_path NORMALIZE "${_implib_path}/../bin")
      
      string(REPLACE ".dll.a" ".dll" _dll_name "${_dll_name}")
      string(REPLACE ".dll" ".pdb" _pdb_name "${_dll_name}")
      get_target_property(_library_name_hints ${library} NAME_HINTS)

      find_program(_dll_path NAMES "${_dll_name}" "${_library_name_hints}" HINTS ${_implib_path} ${_bin_path} NO_CACHE NO_DEFAULT_PATH)
